...
func cloudrunActions() (string, string, string, string) {
	...
	dockerImageName := promptui.Prompt{
		Label:    "Please specify a name for your docker image (for eg: my-app). This name will also be used in Google Cloud Run service",
		Validate: validate,
		Default:  "",
	}
	...
	if onlyBE == "Backend" {
		...
		if answer6 == "Yes" {
			lang := promptui.Select{
				Label: "What language are you using in this project?",
				Items: []string{
...
func dataStreaming() {
	fmt.Println(color.Bold + "PHASE: EVENT STREAMING PLATFORM REDPANDA" + color.Reset)

	redpanda := promptui.Select{
		Label: "Do you want to use Redpanda Event Streaming Platform? (Only for JS projects atm)",
		Items: []string{"Yes", "No"},
	}

	_, includeRedpanda, _ := redpanda.Run()

	if includeRedpanda == "Yes" {
		...
		// parititions -> Data in a topic is spread across a number of partitions. Each partition can be thought of as a log file, ordered by time. To guarantee that you read messages in the correct order, only one instance can read from a particular partition at a time.

		fmt.Printf(color.Red + "\nPAUSE HERE if you dont have any topic to pub/sub yet!\n" + color.Reset)
		fmt.Printf(color.Yellow + "\nYou need to create a topic first by making a POST request to https://thc-redpanda-admin-d7mdbpojaa-as.a.run.app/api/v1/redpanda/createTopic with this payload\n" + payload + color.Reset)
		fmt.Printf(color.Blue + "\n\nThen, you need to install some dependecies: npm i kafkajs uuid dotenv\n" + color.Reset)

		// language == only kafkaJS first, appName == answer2, topic, generate files pub/sub
		topic := promptui.Prompt{
			Label:   "Which topic would you like to publish and subscribe to?",
			Default: "",
		}

		topicName, _ := topic.Run()

		name := promptui.Prompt{
			Label:   "Please specify the name of your application (preferably the same as gcloud service name)",
			Default: answer2,
		}
		...
		extension := "js"
		pub := fmt.Sprintf("producer.%s", extension)
		fpub, errPub := os.Create(pub)

		if errPub != nil {
			log.Fatal(errPub)
		}

		sub := fmt.Sprintf("consumer.%s", extension)
		fsub, errSub := os.Create(sub)

		if errSub != nil {
			log.Fatal(errSub)
		}

		filePub := templates.RedpandaPublish(appName, topicName)
		dataPub := []byte(filePub)

		_, errPubF := fpub.Write(dataPub)

		if errPubF != nil {
			log.Fatal(errPubF)
		}

		fileSub := templates.RedpandaSubscribe(appName, topicName)
		dataSub := []byte(fileSub)

		_, errSubF := fsub.Write(dataSub)
...
func done() {
	...
	fmt.Printf("\np/s: Please reach out to" + color.Blue + " Adri or Ming " + color.Reset + "for the" + color.Yellow + " secrets and brokers IP addresses.\n" + color.Reset)
